...
IS
  ...
  track_no_          VARCHAR2(100) := '';  -- Gemeco Added Tracking information to Email AD 20191114
  ship_detail_       VARCHAR2(100) := '';  -- Gemeco Added Shipping Method Details to Email AD 20191114
  ...
  select
    ...
    group by
     ...
     CURSOR cust_ord_inv_cur(invoice_id_ NUMBER, report_type_like_ VARCHAR2) IS
      SELECT
        ...
        NVL(pi.person_id,'NA')          authorize_code,
        NVL(cmr.value, 'NA')            comm_email,             
        -- cm.name as                      to_name,
        -- cm.value as                     to_email_address
        'Valued Customer' as to_name,
        listagg(cm.value,',') within GROUP (ORDER BY cm.VALUE) as to_email_address
     FROM customer_order_inv_head ih
     LEFT JOIN comm_method_cfv cm ON cm.IDENTITY = ih.IDENTITY AND
         cm.PARTY_TYPE_DB = 'CUSTOMER' AND
         cm.method_id_db = 'E_MAIL' AND
         cm.cf$_amr_email_form_db like report_type_like_
     LEFT JOIN person_info_tab pi ON pi.name = ih.OUR_REFERENCE
     LEFT JOIN comm_method_tab cmr ON cmr.party_type = 'CUSTOMER' 
           AND cmr.method_id = 'E_MAIL' AND cmr.identity = ih.IDENTITY
           AND cmr.name = ih.YOUR_REFERENCE
     WHERE  
         ih.invoice_id = invoice_id_
     GROUP BY
        ih.COMPANY,
        ...
        ih.invoice_date,
        ih.invoice_type,
        ih.name, NVL(pi.person_id,'NA'), NVL(cmr.value, 'NA');
   ...
   -- Gemeco additional shipment details for the email AD 20191114
    CURSOR getShipDet(order_no_ VARCHAR2) IS
      SELECT tracking_number, Shipper_detail FROM gem_shipment_in_tab WHERE delivery_key = order_no_;
  ...
  IF (Event_SYS.Event_Enabled(event_lu_, event_id_)) THEN
     ...
     IF (report_id_ = 'CUSTOMER_ORDER_CONF_REP') THEN
         ...
         str_key1 := archive_parameter_api.get_parameter_value(result_key_, 'ORDER_NO');
          ...
          FOR rec_ IN ord_addr_mail_cur(str_key1, report_type_like_) LOOP
               ...
               Message_SYS.Add_Attribute(msg_, 'KEY_6',              '');
     ...
     ELSIF (report_id_ IN ('CUSTOMER_ORDER_ADV_IVC_REP','CUSTOMER_ORDER_IVC_REP','CUSTOMER_ORDER_COLL_IVC_REP')) THEN
         ...
         FOR rec_ IN cust_ord_inv_cur(num_key1, report_type_like_) LOOP
               msg_          := '';
               to_email_     := NVL(rec_.to_email_address, 'NA');
               
               --No email addresses, just exit.
               IF to_email_ = 'NA' AND rec_.comm_email = 'NA' AND rec_.authorize_code = 'NA' THEN
                 RETURN;
               END IF;

               IF rec_.company = 'GEM' THEN     -- Special Gemeco Processing AD 20191114
                 
                 --Swap out email information and add two more emails  AD 20191114
                 from_name_  := airm1app.person_info_api.get_name('GEMECO');
                 from_email_ := airm1app.Fnd_User_API.Get_Property('GEMECO', 'SMTP_MAIL_ADDRESS');
                 
                 --Add the Reference email, if Exists  AD 20191114
                 IF rec_.comm_email <> 'NA' AND INSTR(rec_.to_email_address, rec_.comm_email) < 1  THEN
                   IF to_email_ = 'NA' THEN
                     to_email_ := rec_.comm_email;
                   ELSE
                     to_email_ := to_email_ || ',' || rec_.comm_email;
                   END IF;
                 END IF;
                 
                 -- Add the Coordinator email, if exists  AD 20191114
                 IF rec_.authorize_code <> 'NA' THEN
                   IF to_email_ = 'NA' THEN
                     to_email_ := airm1app.Fnd_User_API.Get_Property(rec_.authorize_code, 'SMTP_MAIL_ADDRESS');
                   ELSE
                     to_email_ := to_email_ || ',' || airm1app.Fnd_User_API.Get_Property(rec_.authorize_code, 'SMTP_MAIL_ADDRESS');
                   END IF;
                 END IF;
                 
                 OPEN getShipDet(rec_.order_no);
                 FETCH getShipDet INTO track_no_, ship_detail_;
                 IF getShipDet%NOTFOUND THEN
                   track_no_ := 'NA';
                 END IF;
                 CLOSE getShipDet;
                 
               END IF;          -- End Gemeco Specific Details;
               ...
               Message_SYS.Add_Attribute(msg_, 'REPORT_ID',          report_id_);
               Message_SYS.Add_Attribute(msg_, 'SUBJECT',            '');
               Message_SYS.Add_Attribute(msg_, 'TO_EMAIL_ADDRESS',   to_email_);
               Message_SYS.Add_Attribute(msg_, 'TO_NAME',            rec_.to_name);
               Message_SYS.Add_Attribute(msg_, 'KEY_1',              rec_.order_no);
               ...
               Message_SYS.Add_Attribute(msg_, 'KEY_6',              rec_.company);
               Message_SYS.Add_Attribute(msg_, 'KEY_7',              track_no_);
               Message_SYS.Add_Attribute(msg_, 'KEY_8',              ship_detail_);
               Message_SYS.Add_Attribute(msg_, 'KEY_9',              Customer_Order_Api.Get_Customer_Po_No(rec_.order_no));
              
 --               Fnd_Session_Util_API.Impersonate_Fnd_User_('GEMECO');
               -- Send email using event = AMR_SEND_EMAIL
               Event_SYS.Event_Execute(event_lu_, event_id_, msg_); 
